...
const socketIO = require("socket.io")(http, {
  cors: {
    origin: "http://192.168.201.229:3000/",
  },
});
...
socketIO.on("connection", (socket) => {
  ...
  socket.on("newAppointment", (appointmentData) => {
    ...
    if (conversations[conversationId]) {
        ...
        const apiUrl = 'http://192.168.201.229:3000/api/appointments'; // Adjust the port if different
        axios.post(apiUrl, {
          idAppointment:idAppointment,
          Price: price,
          Date: date,
...
function handleAppointmentAction(action, idAppointment, conversationId, socket) {
 ...
 console.log(newStatus)
  ...
  const apiUrl = 'http://192.168.201.229:3000/api/appointments/update-status'; // Adjust the API endpoint
  axios.post(apiUrl, {
    idAppointment : id,
    status: newStatus
  })
