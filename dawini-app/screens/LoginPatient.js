...
const LoginPatient = ({ navigation }) => {
  const { setCurrentUser, role, setRole, setID} = useContext(GlobalContext);
  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      const userId =
        user.role === "patient" ? user.idUser : user["idCare taker"];
      setID(userId);
      ...
      navigation.reset({
        index: 0,
        routes: [
          { name: "MainScreen", params: { idtab: userId, role: user.role } },
        ],
      });
    } catch (error) {
